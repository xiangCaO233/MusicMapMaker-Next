...
/// @brief 菜单再次激活独立辅助窗口后的状态变化。
struct AuxiliaryWindowActivation {
    /// @brief 激活后窗口是否保持打开。
    bool open{ false };
    /// @brief 是否请求将窗口聚焦到前台。
...
constexpr bool resolveAuxiliaryWindowFocusedAndReachable(bool previous,
                                                         bool reachable,
                                                         bool focused,
                                                         bool popupOpen)
{
...
inline bool isAuxiliaryWindowReachable(const AuxiliaryWindowRect& window,
                                       const AuxiliaryWindowRect& workArea,
