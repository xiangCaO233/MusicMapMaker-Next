...
public:
    ...
    void process(ice::AudioBuffer& buffer) override
    {
        ...
        for ( uint16_t ch = 0; ch < m_buffer->num_channels(); ++ch ) {
            size_t frames = std::min(buffer.num_frames(), m_buffer->num_frames());
            std::memcpy(buffer.raw_ptrs()[ch], m_buffer->raw_ptrs()[ch], frames * sizeof(float));
        }
    }
private:
...
static std::shared_ptr<BufferSourceNode> g_bufferSource = std::make_shared<BufferSourceNode>();

AudioWaveformView::AudioWaveformView(const std::string& name)
    : IUIView(name)
{
...
void AudioWaveformView::update(UIManager* sourceManager)
{
    ...
    LayoutContext layoutContext(m_layoutCtx, windowTitle, true, ImGuiWindowFlags_None);
    ...
    ImGui::SliderFloat(TR("ui.waveform.zoom").data(), &m_zoom, 0.1f, 10.0f, "%.1fs");
    ImGui::SameLine();
...
void AudioWaveformView::syncEQ()
{
    ...
    if ( !m_previewEQ || m_previewEQ->get_band_count() != audioManager.getMainTrackEQBandCount() ) {
    ...
    for ( size_t i = 0; i < m_previewEQ->get_band_count(); ++i ) {
        m_previewEQ->set_band_gain_db(i, audioManager.getMainTrackEQBandGain(i));
...
void AudioWaveformView::fullRecalculate()
{
    ...
    size_t totalPoints = static_cast<size_t>(totalTime * m_cachePointsPerSecond) + 1;
    ...
    for ( size_t p = 0; p < totalPoints; ++p ) {
        ...
        if ( m_previewEQ ) {
            ...
            for (uint16_t ch=0; ch<m_rawBuffer->num_channels(); ++ch)
                std::memcpy(m_processBuffer->raw_ptrs()[ch], m_rawBuffer->raw_ptrs()[ch], frames*sizeof(float));
        }
        ...
        m_cachedMinL[p] = minL; m_cachedMaxL[p] = maxL;
        m_cachedMinR[p] = minR; m_cachedMaxR[p] = maxR;
    }

    m_isCalculating = false;
}

void AudioWaveformView::updateEnvelopes(double currentTime, double totalTime, double speed)
{
    ...
    for ( int i = 0; i < m_samplePoints; ++i ) {
        double t = viewStart + (static_cast<double>(i) / m_samplePoints) * (viewEnd - viewStart);
        m_times[i] = t;
