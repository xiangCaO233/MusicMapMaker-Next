...
/// @brief 弹窗提示音的单次触发音量倍率。
constexpr float POPUP_NOTICE_SFX_VOLUME = 1.0f;
...
/// @brief 多行文本编辑动画进度存储键的盐值。
constexpr ImGuiID TEXT_INPUT_PULSE_AMOUNT_KEY_SALT = 0x6D6D4921u;

/// @brief 多行文本编辑动画最后一次绘制帧存储键的盐值。
constexpr ImGuiID TEXT_INPUT_PULSE_LAST_FRAME_KEY_SALT = 0x6D6D4922u;

/// @brief 新增文字残影的初始透明度倍率。
constexpr float TEXT_INPUT_INSERT_GHOST_ALPHA = 0.42f;

/// @brief 删除文字残影的初始透明度倍率。
constexpr float TEXT_INPUT_DELETE_GHOST_ALPHA = 0.62f;

/// @brief 文字增删残影相对当前字号的纵向位移比例。
constexpr float TEXT_INPUT_GHOST_OFFSET_FONT_RATIO = 0.18f;

/// @brief 删除文字残影初始上移量占完整位移的比例。
constexpr float TEXT_INPUT_DELETE_INITIAL_OFFSET_RATIO = 0.35f;

/// @brief 多行文本编辑淡入的最低文字透明度倍率。
constexpr float TEXT_INPUT_MULTILINE_START_ALPHA = 0.80f;

/// @brief 当前活动单行文本框的过渡状态。
struct TextInputTransitionState {
    /// @brief 状态所属的 ImGui 上下文。
    ...
    /// @brief 最近一次由输入框确认的完整文本快照。
    std::string textSnapshot;

    /// @brief 本次编辑删除的 UTF-8 片段。
    std::string deletedText;

    /// @brief 本次编辑插入的 UTF-8 片段。
    std::string insertedText;

    /// @brief 变化片段之前公共文本的显示宽度。
    float prefixWidth{ 0.0f };

    /// @brief 编辑边沿的输入框水平滚动偏移。
    float scrollX{ 0.0f };
...
/// @brief 单一活动文本编辑状态，避免为全部输入框维护常驻映射。
TextInputTransitionState textInputTransitionState;
...
/// @brief 判断字节是否为 UTF-8 连续字节。
/// @param value 待检查字节。
/// @return 属于 UTF-8 连续字节时返回 true。
...
/// @brief 判断单行文本过渡状态是否属于指定输入框。
/// @param context 当前 ImGui 上下文。
/// @param windowId 当前窗口 ID。
/// @param itemId 当前输入框 ID。
/// @return 三项身份均一致时返回 true。
bool isTextInputTransitionStateFor(ImGuiContext* context, ImGuiID windowId,
                                   ImGuiID itemId)
{
    return textInputTransitionState.context == context &&
           textInputTransitionState.windowId == windowId &&
           textInputTransitionState.itemId == itemId;
}

/// @brief 清除当前单行文字残影，不释放文本快照容量。
void clearTextInputTransitionAnimation()
{
    textInputTransitionState.deletedText.clear();
    textInputTransitionState.insertedText.clear();
    textInputTransitionState.prefixWidth = 0.0f;
    textInputTransitionState.scrollX     = 0.0f;
    textInputTransitionState.amount      = 1.0f;
}

/// @brief 为指定输入框同步完整文本快照并清除旧动画。
/// @param context 当前 ImGui 上下文。
/// @param windowId 当前窗口 ID。
/// @param itemId 当前输入框 ID。
/// @param buffer 当前 UTF-8 文本。
/// @warning UI 热路径：仅在活动输入框切换或检测到外部改值时复制文本。
void syncTextInputTransitionSnapshot(ImGuiContext* context, ImGuiID windowId,
                                     ImGuiID itemId, const char* buffer)
{
    textInputTransitionState.context      = context;
    textInputTransitionState.windowId     = windowId;
    textInputTransitionState.itemId       = itemId;
    textInputTransitionState.textSnapshot = buffer ? buffer : "";
    textInputTransitionState.lastFrame    = ImGui::GetFrameCount();
    clearTextInputTransitionAnimation();
}

/// @brief 推进当前单行文本增删动画。
/// @warning UI 热路径：每帧仅进行常量次状态读写和数值计算。
void advanceTextInputTransitionAnimation()
{
    const int currentFrame = ImGui::GetFrameCount();
    if ( textInputTransitionState.lastFrame == currentFrame ) {
        return;
    }
    if ( textInputTransitionState.lastFrame != currentFrame - 1 ) {
        clearTextInputTransitionAnimation();
        return;
    }
    ...
    textInputTransitionState.amount =
        std::min(1.0f, textInputTransitionState.amount + step);
    if ( textInputTransitionState.amount >= 1.0f ) {
        textInputTransitionState.deletedText.clear();
        textInputTransitionState.insertedText.clear();
    }
}

/// @brief 从新旧 UTF-8 文本提取变化片段并启动单行文字动画。
/// @param previousText 编辑前文本。
/// @param currentText 编辑后文本。
/// @warning UI 编辑边沿路径：只在文本实际变化时线性扫描并复制变化片段。
void startTextInputTransition(const std::string& previousText,
                              const char*        currentText)
{
    const std::string nextText = currentText ? currentText : "";
    if ( previousText == nextText ) {
        return;
    }

    const std::size_t sharedSize =
        std::min(previousText.size(), nextText.size());
    std::size_t prefixLength = 0;
    while ( prefixLength < sharedSize &&
            previousText[prefixLength] == nextText[prefixLength] ) {
        ++prefixLength;
    }
    while ( prefixLength > 0 &&
            ((prefixLength < previousText.size() &&
              isUtf8ContinuationByte(previousText[prefixLength])) ||
             (prefixLength < nextText.size() &&
              isUtf8ContinuationByte(nextText[prefixLength]))) ) {
        --prefixLength;
    }

    std::size_t previousEnd = previousText.size();
    std::size_t nextEnd     = nextText.size();
    while ( previousEnd > prefixLength && nextEnd > prefixLength &&
            previousText[previousEnd - 1] == nextText[nextEnd - 1] ) {
        --previousEnd;
        --nextEnd;
    }
    while ( previousEnd < previousText.size() && nextEnd < nextText.size() &&
            (isUtf8ContinuationByte(previousText[previousEnd]) ||
             isUtf8ContinuationByte(nextText[nextEnd])) ) {
        ++previousEnd;
        ++nextEnd;
    }

    textInputTransitionState.deletedText.assign(
        previousText, prefixLength, previousEnd - prefixLength);
    textInputTransitionState.insertedText.assign(
        nextText, prefixLength, nextEnd - prefixLength);
    textInputTransitionState.prefixWidth =
        ImGui::CalcTextSize(
            previousText.data(), previousText.data() + prefixLength, false)
            .x;
    textInputTransitionState.amount    = 0.0f;
    textInputTransitionState.lastFrame = ImGui::GetFrameCount();
}

/// @brief 读取指定活动输入框的水平滚动偏移。
/// @param itemId 输入框 ID。
/// @return 当前原生输入状态的横向滚动值，非活动时返回 0。
/// @warning UI 热路径：只读 ImGui 当前文本编辑状态，不写入光标或选区。
float getTextInputScrollX(ImGuiID itemId)
{
    const ImGuiInputTextState* inputState = ImGui::GetInputTextState(itemId);
    return inputState ? inputState->Scroll.x : 0.0f;
}

/// @brief 在原生单行输入框上绘制增删文字残影。
/// @param frameMin 输入框左上角。
/// @param frameMax 输入框右下角。
/// @warning UI 热路径：动画期间最多追加两段经过裁剪的文字，不提交 Item。
void drawTextInputTransition(const ImVec2& frameMin, const ImVec2& frameMax)
{
    if ( textInputTransitionState.amount >= 1.0f ||
         (textInputTransitionState.deletedText.empty() &&
          textInputTransitionState.insertedText.empty()) ) {
        return;
    }

    const ImGuiStyle& style = ImGui::GetStyle();
    const ImVec4      clipRect{ frameMin.x + style.FramePadding.x,
                                frameMin.y + style.FramePadding.y,
                                frameMax.x - style.FramePadding.x,
                                frameMax.y - style.FramePadding.y };
    if ( clipRect.z <= clipRect.x || clipRect.w <= clipRect.y ) {
        return;
    }

    const float eased     = easeOutCubic(textInputTransitionState.amount);
    const float remaining = 1.0f - eased;
    const float offsetY   = std::max(
        2.0f, ImGui::GetFontSize() * TEXT_INPUT_GHOST_OFFSET_FONT_RATIO);
    const ImVec2 textBase{
        frameMin.x + style.FramePadding.x +
            textInputTransitionState.prefixWidth -
            textInputTransitionState.scrollX,
        frameMin.y + style.FramePadding.y,
    };
    const ImVec4 baseColor = ImGui::GetStyleColorVec4(ImGuiCol_Text);
    ImDrawList*  drawList  = ImGui::GetWindowDrawList();

    const auto drawGhost =
        [&](const std::string& text, ImVec2 position, float alpha) {
    ...
    drawGhost(
        textInputTransitionState.deletedText,
        ImVec2(textBase.x,
               textBase.y -
                   offsetY * (TEXT_INPUT_DELETE_INITIAL_OFFSET_RATIO +
                              (1.0f - TEXT_INPUT_DELETE_INITIAL_OFFSET_RATIO) *
                                  eased)),
        TEXT_INPUT_DELETE_GHOST_ALPHA * remaining);
    drawGhost(textInputTransitionState.insertedText,
              ImVec2(textBase.x, textBase.y + offsetY * remaining),
              TEXT_INPUT_INSERT_GHOST_ALPHA * remaining);
}

/// @brief 读取并推进多行文本整体淡入进度。
/// @param itemId 输入框 ID。
/// @param storage 输入框所属窗口状态存储。
/// @return 本帧绘制应使用的线性动画进度。
/// @warning UI 热路径：只访问当前窗口 ImGuiStorage。
float updateTextInputPulseAmount(ImGuiID itemId, ImGuiStorage* storage)
{
    if ( !storage ) {
        return 1.0f;
    }
    ...
    const ImGuiID amountKey =
        makeButtonStorageKey(itemId, TEXT_INPUT_PULSE_AMOUNT_KEY_SALT);
    const ImGuiID lastFrameKey =
        makeButtonStorageKey(itemId, TEXT_INPUT_PULSE_LAST_FRAME_KEY_SALT);
    const int currentFrame = ImGui::GetFrameCount();
    const int lastFrame    = storage->GetInt(lastFrameKey, -1);
    if ( lastFrame == currentFrame ) {
        return storage->GetFloat(amountKey, 1.0f);
    }

    float       amount = lastFrame == currentFrame - 1
                             ? storage->GetFloat(amountKey, 1.0f)
                             : 1.0f;
    const float step   = std::max(0.0f, ImGui::GetIO().DeltaTime) *
                         Utils::getUiAnimationTransitionSpeed();
    return std::min(1.0f, amount + step);
}

/// @brief 写回多行文本淡入进度，并在编辑边沿重新启动动画。
/// @param itemId 输入框 ID。
/// @param storage 输入框所属窗口状态存储。
/// @param amount 当前动画进度。
/// @param edited 本帧是否实际编辑了文本。
/// @warning UI 热路径：只写入当前窗口 ImGuiStorage。
void finishTextInputPulse(ImGuiID itemId, ImGuiStorage* storage, float amount,
                          bool edited)
{
    if ( !storage ) {
        return;
    ...
    const ImGuiID lastFrameKey =
        makeButtonStorageKey(itemId, TEXT_INPUT_PULSE_LAST_FRAME_KEY_SALT);
    storage->SetFloat(amountKey, edited ? 0.0f : amount);
    storage->SetInt(lastFrameKey, ImGui::GetFrameCount());
}

/// @brief 计算多行文本编辑后的整体淡入颜色。
/// @param amount 线性动画进度。
/// @return 当前帧文字颜色。
ImVec4 calcTextInputPulseColor(float amount)
{
    ImVec4 color = ImGui::GetStyleColorVec4(ImGuiCol_Text);
    color.w *= TEXT_INPUT_MULTILINE_START_ALPHA +
               (1.0f - TEXT_INPUT_MULTILINE_START_ALPHA) * easeOutCubic(amount);
    return color;
}

/// @brief 调用原生单行文本输入并叠加增删文字过渡。
/// @tparam DrawInput 原生 InputText 调用闭包类型。
/// @param label 输入框标签和 ID。
/// @param buffer UTF-8 文本缓冲区。
/// @param flags 文本输入标志。
/// @param drawInput 原生输入调用闭包。
/// @return 原生输入控件返回值。
/// @warning UI 热路径：只跟踪当前活动输入框；文本复制与差异计算仅发生在
/// 激活切换、外部改值或实际编辑边沿。
template<typename DrawInput>
bool animatedSingleLineInputText(const char* label, char* buffer,
                                 ImGuiInputTextFlags flags,
                                 DrawInput&&         drawInput)
{
    ImGuiContext* context  = ImGui::GetCurrentContext();
    ImGuiWindow*  window   = ImGui::GetCurrentWindow();
    const ImGuiID windowId = window ? window->ID : 0;
    const ImGuiID itemId   = ImGui::GetID(label);
    const bool    skipTransition =
        (flags & (ImGuiInputTextFlags_Password |
                  ImGuiInputTextFlags_CallbackResize)) != 0;

    if ( skipTransition ) {
        if ( isTextInputTransitionStateFor(context, windowId, itemId) ) {
            textInputTransitionState = {};
        }
        return std::forward<DrawInput>(drawInput)();
    }

    const ImVec2 frameMin = ImGui::GetCursorScreenPos();
    const ImVec2 frameMax{ frameMin.x + ImGui::CalcItemWidth(),
                           frameMin.y + ImGui::GetFrameHeight() };
    bool         stateMatches =
        isTextInputTransitionStateFor(context, windowId, itemId);
    if ( ImGui::GetActiveID() == itemId && !stateMatches ) {
        syncTextInputTransitionSnapshot(context, windowId, itemId, buffer);
        stateMatches = true;
    }
    if ( stateMatches ) {
        if ( textInputTransitionState.textSnapshot != (buffer ? buffer : "") ) {
            syncTextInputTransitionSnapshot(context, windowId, itemId, buffer);
        } else {
            advanceTextInputTransitionAnimation();
        }
    }

    const bool changed = std::forward<DrawInput>(drawInput)();
    const bool edited  = ImGui::IsItemEdited();
    const bool active  = ImGui::IsItemActive();
    stateMatches = isTextInputTransitionStateFor(context, windowId, itemId);

    if ( active && !stateMatches ) {
        syncTextInputTransitionSnapshot(context, windowId, itemId, buffer);
        stateMatches = true;
    } else if ( stateMatches && edited &&
                textInputTransitionState.textSnapshot !=
                    (buffer ? buffer : "") ) {
        startTextInputTransition(textInputTransitionState.textSnapshot, buffer);
        textInputTransitionState.textSnapshot = buffer ? buffer : "";
    }

    if ( stateMatches ) {
        if ( active ) {
            textInputTransitionState.scrollX = getTextInputScrollX(itemId);
        }
        drawTextInputTransition(frameMin, frameMax);
        textInputTransitionState.lastFrame = ImGui::GetFrameCount();
    }
    return changed;
}
...
void FeedbackOpenPopup(const char* popupId)
{
    ...
    if ( !wasOpen && ImGui::IsPopupOpen(popupId) ) {
...
void PlayPopupOpenFeedback()
{
    ...
    Audio::AudioManager::instance().playSoundEffect(POPUP_NOTICE_SFX_KEY,
                                                    POPUP_NOTICE_SFX_VOLUME);
}
...
{
    return animatedSingleLineInputText(label, buffer, flags, [&] {
        return ImGui::InputText(
            label, buffer, bufferSize, flags, callback, userData);
    });
}
                               ...
                               ImGuiInputTextCallback callback, void* userData)
{
    return animatedSingleLineInputText(label, buffer, flags, [&] {
        return ImGui::InputTextWithHint(
            label, hint, buffer, bufferSize, flags, callback, userData);
    });
}
...
bool AnimatedInputTextMultiline(const char* label, char* buffer,
                                std::size_t bufferSize, const ImVec2& size,
                                ImGuiInputTextFlags    flags,
                                ImGuiInputTextCallback callback, void* userData)
{
    ImGuiStorage* storage = ImGui::GetStateStorage();
    const ImGuiID itemId  = ImGui::GetID(label);
    const float   amount  = updateTextInputPulseAmount(itemId, storage);
    ImGui::PushStyleColor(ImGuiCol_Text, calcTextInputPulseColor(amount));
    const bool changed = ImGui::InputTextMultiline(
        label, buffer, bufferSize, size, flags, callback, userData);
    const bool edited = ImGui::IsItemEdited();
    ImGui::PopStyleColor();
    finishTextInputPulse(itemId, storage, amount, edited);
    return changed;
}
