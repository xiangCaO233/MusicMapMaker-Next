-- RM：从 EX Rhythm Master VI 的实际游玩资源迁移。
-- 原素材作者为 dreamcat、Acc@9961、Sam_0324；迁移说明见同目录 README.md。
...
local function audio(path)
    return { path = path, lead_in_ms = 0.0 }
end
...
return {
    ...
    -- 原包序列帧按 60 FPS 播放，保持短促的蓝绿打击闪光。
    meta = {
        name = "RM",
        author = "dreamcat, Acc@9961, Sam_0324 / MusicMapMaker-Next 迁移",
        version = "1.0",
        effectbasefps = 60,
    },
    ...
    audios = {
        -- 两种按键音直接取游玩脚本 Audio:Load 指定的文件。
        -- OGG 保持原编码；零提前量避免把未知延迟当成校准值。
        hiteffect = {
            note = audio(resource("audio/note.ogg")),
            flick = audio(resource("audio/flick.ogg")),
        },
    ...
    assets = {
        ...
        note = {
            -- 全部音符组件来自同一图集；两张按键统一为 256 × 112 的布局矩形。
            ...
            -- 所有组件从归一化图集统一缩小为 50%，避免各自缩放改变光晕范围。
            ...
            effect = {
                ...
                note = resource("image/note/effect/note/[1 .. 17].png"),
                flick = resource("image/note/effect/flick/[1 .. 18].png"),
                -- 后半段保留稳定光团与流动粒子，长按不重复单键的消散过程。
                hold = resource("image/note/effect/flick/[10 .. 18].png"),
            },
        },
    },
