...
#
#   A) 预合并文件   -DMMM_PGO_DATA=merged.profdata
#   B) 本地目录     -DMMM_PGO_SOURCE_DIR=/path/to/profiles  (自动合并 *.profraw)
#   C) 远程 URL     -DMMM_PGO_SOURCE_URL=https://server.com/pgo/  (自动下载+合并)
#
#  三种构建模式:
#
#  1) 纯插桩 (MMM_PGO_INSTRUMENT=ON):
#     cmake -B build_pgo -G Ninja -DCMAKE_BUILD_TYPE=RelWithDebInfo
#       -DMMM_PGO_INSTRUMENT=ON
#       -DMMM_PGO_UPLOAD_URL="https://your-server.com/pgo/upload"
#
#  2) 纯优化 (MMM_PGO_USE=ON + 数据源):
#     cmake -B build_pgo -G Ninja -DCMAKE_BUILD_TYPE=Release
#       -DMMM_PGO_USE=ON
#       -DMMM_PGO_SOURCE_URL="https://your-server.com/pgo/"
#     # 或:
#     cmake -B build_pgo -G Ninja -DMMM_PGO_USE=ON
#       -DMMM_PGO_SOURCE_DIR=C:/profiles
#     # 或:
#     cmake -B build_pgo -G Ninja -DMMM_PGO_USE=ON
#       -DMMM_PGO_DATA=merged.profdata
#
#  工作流循环 (profile 在版本间累积):
#    周期 N  : 插桩版 (Build A) → 分发用户 → 服务器合并 profile[0..N]
#    周期 N+1: 优化版 (Build B) → 使用 profile[0..N] 编译最终发行版
#    同时继续: 插桩版 (Build A) → 分发更多用户 → 扩大 profile 覆盖
#
#    Build A 和 Build B 是两个独立构建目录，不能合并！
#    Clang 不允许 -fprofile-instr-use 和 -fprofile-instr-generate 同时用。
#    每轮把 Build B 的 -DMMM_PGO_SOURCE_DIR/URL 指向服务器合并后的
...
if(MMM_PGO_USE)
    ...
    if(NOT "${MMM_PGO_DATA}" STREQUAL "")
        ...
        if(MMM_PGO_DATA MATCHES "^https?://")
            ...
            file(DOWNLOAD "${MMM_PGO_DATA}" "${_downloaded}" STATUS _dl_status TIMEOUT 60)
...
if(MMM_PGO_INSTRUMENT)
    ...
    if(MMM_PGO_UPLOAD_URL)
        ...
        configure_file(
            "${CMAKE_CURRENT_SOURCE_DIR}/cmake/pgo_upload_url.h.in"
            "${CMAKE_BINARY_DIR}/generated/pgo_upload_url.h"
            @ONLY
        )
    else()
        message(WARNING "PGO: MMM_PGO_UPLOAD_URL not set. Profiles saved locally only.")
    endif()
endif()
