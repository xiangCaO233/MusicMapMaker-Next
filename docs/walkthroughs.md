...
`compose-beatmap.json` 是创作章节的第四阶段。进入时必须有活动项目和已打开的真实谱面编辑器标签；主题采用 `completion: all`，一个分支中的九步按顺序衔接：
...
7. 继续按住 Shift，在单 Note 附近的另一轨沿纵向拖出 Hold；起点和终点对齐实际分拍线。
8. 用绘制工具右键删除刚绘制的 Hold。
9. 逐个删除本轮练习的其余 Note 和 Flick。
