...
`editor-personalization.json` 同属“个性化”章节，但需要打开项目及谱面。先选择布局工具，再解释可选画布组件的允许区域：通常保持内容在画布内，拍号和分拍线时间还受所属时间格限制；悬停或拖动组件时画布遮罩会显示实际范围。随后分别引导当前判定线时间、拍号、分拍线时间、背景频谱和 KPS，接着调整判定线位置、主轨道宽度、草稿区和 BGM 区各自的横向位置及宽度，最后调整 Note 的宽高倍率。位置和字号均由用户按习惯决定，每步都可自行点“知道了”；可选组件尚未开启时高亮对应显隐开关，布局设置面板在编辑器个性化阶段保持可见；当前没有可见 Note 时，步骤文字说明可改用面板滑块。
