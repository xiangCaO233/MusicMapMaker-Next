...
`compose-beatmap.json` 是创作章节的第四阶段。进入时必须有活动项目和已打开的真实谱面编辑器标签；主题采用 `completion: all`，一个分支中的十步按顺序衔接：
...
9. 用绘制工具右键删除刚绘制的 Hold。
10. 逐个删除本轮练习的其余折线、Note 和 Flick；删除整条折线时按住 Shift 再右键。

放置目标来自画布实际显示的分拍线，随滚动和窗口布局更新。放置失败会取消本次临时画笔，不撤销此前的编辑；成功放置的物件进入正常编辑历史。删除步骤按本轮创建的物件身份和逻辑快照确认结果，不能仅凭一次右键或物件暂时离开视口完成。“返回上一步”会回滚对应的练习成果；离开引导后，已完成的学习进度仍保存在独立的进度文件中。
