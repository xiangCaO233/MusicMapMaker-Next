# MusicMapMaker-Next 插件系统

插件系统从用户配置目录载入可热重载的 Lua 扩展。目前提供 `theme` 主题插件
与 `tool` 工具插件。主题插件调整 Dear ImGui 外观；工具插件通过声明式控件
创建可停靠窗口，并调用文件选择、音频处理和谱面转换等宿主能力。

- 主题插件的字段与示例见 [`theme-plugin.md`](theme-plugin.md)。
- 工具插件的 API、示例和性能边界见 [`tool-plugin.md`](tool-plugin.md)。

## 目录位置

应用通过 `MMM::Config::AppPaths` 计算目录，不依赖当前工作目录：

- 插件根目录：`<配置根目录>/plugins`
- 主题插件目录：`<配置根目录>/plugins/themes`
- 工具插件目录：`<配置根目录>/plugins/tools`
- Linux/macOS 默认配置根目录：`~/.config/mmm`
- Linux/macOS 设置了 `XDG_CONFIG_HOME` 时：`$XDG_CONFIG_HOME/mmm`
- Windows：`%USERPROFILE%/.config/mmm`

启动应用或首次重载时会自动创建插件目录。主题目录内的 `.lua` 文件会被递归
扫描，所以主题插件可以放在独立子目录中，例如：

```text
plugins/
└── themes/
    └── twilight/
        └── theme.lua
```

工具插件在 `plugins/tools/` 下按文件扫描，一份 `.lua` 文件定义一个窗口，
目前不递归扫描子目录。内置音频转码与谱面转换工具无需复制文件即可在“工具 →
插件列表”中打开；对应源码位于仓库 `assets/plugins/tools/`。

仓库内可直接复制的示例：

- [`examples/theme-example.lua`](examples/theme-example.lua)：完整字段与颜色注释模板；
- [`examples/ivm.lua`](examples/ivm.lua)：内置 `IVM` 主题的 Lua 插件接口
  移植示例，加载后显示为 `IVM（Lua 示例）`；
- [`examples/ivm.mmpalette`](examples/ivm.mmpalette)：与 `IVM` 主题配套的
  物件及分拍线配色，使用调色盘弹窗中的“导入方案”载入。

`IVM` 已作为内置主题注册，不安装示例插件也可直接选择。配套的独立内置
皮肤位于 [`assets/skins/ivm`](../../assets/skins/ivm)，在自动主题模式下
无论系统偏亮还是偏暗都固定选用 `IVM`。

外置 `IVM` 配色中的普通物件使用青色；头部、Body、尾部、滑键箭头和折线
节点统一使用同一组荧光绿 RGBA，其中节点颜色与 Body 完全一致。该可选配色
保留红色整拍线；IVM 皮肤自身同样使用纯红色拍头线，其余分拍线固定为同一
组完全不透明灰色。

## 主题插件生命周期

1. 应用图形上下文完成 ImGui 与字体初始化。
2. 注册所有编译进程序的内置主题实例。
3. 清除内存中此前由插件创建的全部主题实例。
4. 递归扫描 `plugins/themes`，按 UTF-8 路径排序并执行每个 `.lua` 文件。
5. 校验插件返回值，为每个有效定义创建独立主题实例并加入主题列表。
6. 重新应用设置中保存的主题稳定 ID；ID 不存在时回退到 `DeepDark`。

运行期间可点击“工具 → 重载插件”重复执行第 3 至第 6 步。内置主题不会被
删除；已从磁盘移除或改名的自定义主题会在这次重载后从列表消失。重载结果会
显示成功载入的主题数和错误数，详细错误写入应用日志。

“工具 → 插件列表”会显示最近一次扫描到的每个 Lua 文件、启用状态、创建的
主题数和首个加载错误。复选框以插件文件为粒度热开关：

- 关闭后立即删除该文件创建的全部主题实例，并重新载入其他启用插件；
- 开启后立即执行该文件并恢复其中的有效主题实例；
- 一个文件通过 `themes` 定义多个主题时，这些主题会一起启用或禁用；
- 开关以 `themes/<相对 Lua 路径>` 保存到 `user_config.json`，重启后继续生效；
- 移动或重命名 Lua 文件会产生新的插件 ID，因此会恢复为默认启用状态。

如果关闭的是当前选中主题所属插件，界面会暂时回退到 `DeepDark`，但保留原
主题 ID；再次启用插件后会自动恢复该主题。

## 主题插件约束

- 插件是本地 Lua 代码，会在加载时执行，只应安装可信来源的插件。
- 每个插件文件使用独立 Lua 状态，不能依赖另一个插件留下的全局变量。
- 当前开放 `base`、`table`、`string`、`math` Lua 标准库。
- 加载失败只跳过对应文件或主题定义，不影响其他插件继续载入。
- 禁用插件仍会出现在插件列表中，但其 Lua 文件不会执行。
- 插件实例只存在于内存；主题选择以稳定字符串 ID 保存到
  `user_config.json`。

主题的完整返回结构、字段类型、颜色名和多主题写法见
[`theme-plugin.md`](theme-plugin.md)。

工具插件有独立的加载器、窗口生命周期和宿主 API，详见
[`tool-plugin.md`](tool-plugin.md)。主题插件列表中的启用开关只控制主题脚本；
工具插件通过新增或移除文件，再点击“工具 → 重载插件”更新。
