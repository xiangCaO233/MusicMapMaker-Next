...
内置音频工具提供 FLAC、WAV、MP3、Ogg、M4A、Opus 和 AAC 输出格式选择，
保存建议名随格式变化。保存对话框中手动输入已知后缀时，工具会同步格式选择；
没有后缀或使用未知后缀时需要重新选择目标文件。
`.ogg` 明确使用 Vorbis，`.opus` 使用 Ogg 封装的 Opus，避免 FFmpeg 构建选项
改变 Ogg 的默认编码器后使目标码率被当作无损参数拒绝。
切换已有输出文件的格式后需要重新选择目标，以重新确认实际写入路径。
FLAC 和 WAV 不显示目标码率输入，导出时传入 `0`；切回有损格式会保留此前输入
的码率。下拉选项表示可尝试的格式，最终能否编码仍由当前 FFmpeg 构建决定。
