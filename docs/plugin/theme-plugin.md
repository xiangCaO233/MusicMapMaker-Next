...
“工具 → 插件列表”按 Lua 文件展示并切换插件。关闭一个文件时，该文件当前
创建的所有主题实例会被删除，其他启用插件会重新载入；重新打开时则立即执行
该文件并恢复有效主题。开关状态保存在 `user_config.json` 的
`settings.disabledPluginIds` 数组中，例如：

```json
{
    "settings": {
        "disabledPluginIds": [
            "themes/twilight/theme.lua"
        ]
    }
}
```

插件 ID 使用相对于插件根目录的通用路径分隔符，与主题定义中的 `id` 不同：
前者标识一个 Lua 文件并控制是否执行，后者标识该文件创建的单个主题实例。
