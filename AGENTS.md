...
- **修改后检查**：修改自维护代码时应复查全项目逐文件统计，交付时报告统计范围，并逐一列出本次新增或修改文件的路径、注释行数、代码行数和比例；其他未达标文件应提供清单。任一文件未达到 30% 时必须明确报告，不得用整体达标掩盖单文件不足；补充注释仍需遵守最小化修改原则，跨模块的大规模补注释应作为独立任务获得用户授权，不得擅自扩大当前任务范围。
